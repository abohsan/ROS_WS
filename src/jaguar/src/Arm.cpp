...
Arm::~Arm() {
    ...
    //     delete tcpRobot3;
    // }
    // std::cout << "JaguarArm::~JaguarArm()"  << std::endl;
}

void Arm::executeCommand(const char &cmd) {
    // if ( cmd == '0' ){
    //     motorStopAll();
    // } else if ( cmd == '1' ){
    //     motorReleaseAll();
    // } else if ( cmd == '9' ) {  // still not working
    //     motor1Up();
    // } else if ( cmd == '8' ) { // still not working
    //     motor1Down();
    // } else if ( cmd == 'j' ) {
    //     motor2Up();
    // } else if ( cmd == 'n' ) {
...
void Arm::connectToRobot1() {
    // if ( !m_motor1Connected ) {
    //     //        // Motor address
    //     QString addr = "192.168.0.63";
    //     quint16 port = 10001;
    //     //        // TCP/IP connection
    //     m_tcpRobot1 = new QTcpSocket(this);
    //     m_tcpRobot1->connectToHost(addr,port);
    //     connect(m_tcpRobot1,SIGNAL(readyRead()), this, SLOT(processRobotData1()));
    //     sendQuery1();
    ...
    //     //        m_pingTimer.start();
    //     m_watchDogCnt1 = 2;
    //     m_motor1Connected = true;
    // } else {
    //     m_pingTimer.stop();
    //     disconnect(&m_pingTimer, SIGNAL(timeout()), this, SLOT(sendPing()));
    //     m_tcpRobot1->close();
    //     tcpRobot2->close();
    //     m_watchDogCnt1 = 2;
    //     m_motor1Connected = false;
    // }
    // std::cout << "JaguarArm::connectToRobot1()"  << std::endl;
}

void Arm::connectToRobot2()
{
    // if (m_motor1Connected2)
    // {

    //     QString addr = "192.168.0.62";
    //     quint16 port = 10001;
...
void Arm::processRobotData1() {
    ...
    //     received = m_tcpRobot1->readAll();
    //     m_receivedData1.append(received);
    // }
    // if ( m_receivedData1.endsWith("\r") ) {
...
void Arm::motor1Down()
{
    // if (m_tcpRobot1 != nullptr){
    //     if (m_tcpRobot1->isWritable()){
    //         if (armIniFlag){
    //             if ((jointMotorData[0].encoderPos - J1_CMD) >= jointMotorData[0].limitPos1)
    //             {
    //                 m_tcpRobot1->write(J1_DOWN_CMD);
    //             }
    //         }
    //         else{
    //             m_tcpRobot1->write(J1_DOWN_CMD);
...
void Arm::sendQuery1() {
    // if ( m_tcpRobot1 ) {
    //     if (m_tcpRobot1->isWritable())
    //         m_tcpRobot1->write("# C_?A_?A_?AI_?C_?FF_?P_?S_?T_?V_# 20\r");
    // }
}
...
void Arm::cmdSend(int channel,int cmdValue,int motorCtrl)
{
    ...
    //     {
    //         temp = "!PR 1 " + temp + "\r";
    //         m_tcpRobot1->write(temp.toLatin1().data());
    //     }
...
void Arm::sendPing() {
    ...
    //     m_tcpRobot1->write("~MMOD\r");
    //     tcpRobot2->write("~MMOD\r");
    // }
    // else
    // {
    //     if (m_watchDogCnt1> 10)
    //     {
...
void Arm:: j1GoCmd()
{
    ...
    //                     m_tcpRobot1->write(cmdByte.data());
    //                 }
    //             }
    //         }
    //     }
